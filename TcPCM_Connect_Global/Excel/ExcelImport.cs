                    ...
                    foreach (DataGridViewColumn col in dgv.Columns)
                    {
                        for (int category = xlRng.GetLowerBound(0); category < keys.Length; category++)
                        {
                            ...
                            if (keys[category].Contains(col.Name) && !keys.Where(key => key != keys[category]).Contains(col.Name))
                            {
                                ...
                                if (new List<string>() { "지역", "공정", "업종", "설비명", "설비구분", "통화", "Valid From", "구분자", "재질명", "소재명", "메이커", "구분", "비중", "비중 단위", "가격 단위", "원재료 단위", "스크랩 단위", "이름", "ISO", "UOM Code", "UOM 명", "Plant", "GRADE" }.Contains(col.Name)) continue;
